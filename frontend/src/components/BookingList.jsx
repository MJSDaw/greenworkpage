...
const BookingList = () => {
  ...
  const handleChange = (e) => {
    ...
    setFormData(prevData => {
      ...
      if (name === 'selected_date') {
        ...
        console.log('Getting slots for date:', value, 'and space:', newData.space_id);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (!formData.user_id || !formData.space_id || !formData.selected_date || !formData.start_time || !formData.end_time) {
        setErrors({
  ...
  const handleEditClick = (id) => {
    if (editingId === id) {
      ...
      const bookingToEdit = bookings.find((booking) => {
        const bookingId = booking.id || `${booking.user_id}-${booking.space_id}-${booking.reservation_period}`;
        return bookingId === id;
      });
      ...
      setFormData({
        ...
        end_time: endTime || '',
        reservation_period: startDate && startTime && endTime ? `${startDate} ${startTime}-${startDate} ${endTime}` : ''
      });
  ...
  const getAvailableSlots = (date, spaceId) => {
    ...
    const dateBookings = existingBookings.filter(b => {
      ...
      const bookingId = b.id || `${b.user_id}-${b.space_id}-${b.reservation_period}`;
    ...
    schedules.forEach(schedule => {
      ...
      dateBookings.forEach(booking => {
        ...
        if (booking.reservation_period.includes('|')) {
          ...
          const [bookingStart, bookingEnd] = booking.reservation_period.split('-').map(part => part.trim());
        ...
        console.log('Processing booking:', {
          ...
          isCurrentBooking: editingId === (booking.id || `${booking.user_id}-${booking.space_id}-${booking.reservation_period}`)
        });
        
        // Solo agregamos los puntos de tiempo si no es la reserva que estamos editando
        if (editingId !== (booking.id || `${booking.user_id}-${booking.space_id}-${booking.reservation_period}`)) {
          timePoints.push({
                ...
                <article className="card">
                  <div className="card__content">
                    <div className="card__text">
                      ...
                      <p>
                        {t('common.period')}: {booking.start_date || booking.reservation_period?.split('-')[0]} - {booking.end_date || (booking.reservation_period ? booking.reservation_period.split('-')[1] : '')}
                      </p>
                    </div>
                  </div>
                  ...
                  <article className="card--form--edit">
                    <form onSubmit={handleSubmit}>
                      <div className="form__section">
                        ...
                        {errors.user_id && Array.isArray(errors.user_id) &&
                          errors.user_id.map((err, idx) => (
                            <span className="form__error" key={idx}>{t(`errors.${err}`)}</span>
                          ))}
                      </div>
                      ...
                      <div className="form__section">
                        ...
                        {errors.space_id && Array.isArray(errors.space_id) &&
                      ...
                      <div className="form__section">
                        ...
                        <input
                          ...
                          required
                        />
                        {errors.selected_date && Array.isArray(errors.selected_date) &&
                      ...
                      {formData.selected_date && availableSchedules.length > 0 && (
                        <>
                          <div className="form__section">
                            <label>Rangos de tiempo disponibles:</label>
                            <div className="schedule-display">
                              {availableSchedules.map((range, index) => (
                                <div key={index}>{range.start} - {range.end}</div>
                              ))}
                            </div>
                          </div>
                          ...
                          <div className="form__section">
                            ...
                            >
                              ...
                              {parseSpaceSchedule(selectedSpace.schedule)
                                .filter(s => s.day === getDayOfWeek(formData.selected_date))
                                .map((schedule, index) => {
                            ...
                            <div className="form__section">
                              ...
                              >
                                ...
                                {parseSpaceSchedule(selectedSpace.schedule)
                                  .filter(s => s.day === getDayOfWeek(formData.selected_date))
                                  .map((schedule, index) => {
                                    if (timeToMinutes(schedule.start) <= timeToMinutes(formData.start_time) &&
                                        timeToMinutes(schedule.end) > timeToMinutes(formData.start_time)) {
                                      const startMinutes = timeToMinutes(formData.start_time) + 60;
                                      const endMinutes = timeToMinutes(schedule.end);
                                      const options = [];
                                      for (let time = startMinutes; time <= endMinutes; time += 60) {
                                        const timeStr = minutesToTime(time);
                      ...
                      {formData.selected_date && availableSchedules.length === 0 && (
                        <div className="form__section">
                          <p className="form__error">No available time slots for the selected date.</p>
                          {existingBookings.filter(b => 
                            ...
                            <p key={index} className="booking-info">
                              Reserved: {booking.reservation_period.split(' ')[1]} - {booking.reservation_period.split(' ')[3]}
                            </p>
                          ))}
                        </div>
                      )}
