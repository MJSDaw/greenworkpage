...
El proyecto está organizado en las siguientes carpetas para una mejor estructura:

### Carpetas Principales

- `api/`: Contiene todo el código del backend Laravel
- `frontend/`: Contiene todo el código del frontend React + Vite
- `config/`: Archivos de configuración globales (Apache, pgAdmin, etc.)
- `scripts/`: Scripts de utilidad para la gestión del proyecto
- `docs/`: Documentación adicional del proyecto
- `backups/`: Copias de seguridad de la base de datos
- `ssl/`: Certificados SSL para HTTPS
...
En la carpeta `scripts/` encontrarás herramientas útiles para trabajar con el proyecto:

- `start-project.bat` / `start-project.sh`: Inicia todos los servicios
- `check-https.bat` / `check-https.sh`: Verifica la configuración HTTPS
- `setup-project-structure.bat` / `setup-project-structure.sh`: Asegura que la estructura de directorios sea correcta
...
Consulta la carpeta `docs/` para acceder a documentación adicional:

- `HTTPS-CONFIG.md`: Detalles sobre la configuración HTTPS
- `HTTPS-SUMMARY.md`: Resumen de los cambios para implementar HTTPS
